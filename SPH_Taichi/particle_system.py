...
class ParticleSystem:
    def __init__(self, config: SimConfig, GGUI=False, max_num_particles=None):
        self.cfg = config
        self.GGUI = GGUI
        ...
        # domain_end should come from config; fix previous typo
        ...
        # Custom boundary boxes
        self.boundary_boxes_start = []
        self.boundary_boxes_end = []
        self.num_boundary_boxes = 0
        ...
        #### Process Custom Boundary Boxes ####
        boundary_conditions = self.cfg.get_boundary_conditions()
        for bc in boundary_conditions:
            if bc.get("type") == "bounding_box":
                box_start = np.array(bc["point"]) - np.array(bc["size"]) / 2.0
                box_end = np.array(bc["point"]) + np.array(bc["size"]) / 2.0
                self.boundary_boxes_start.append(box_start)
                self.boundary_boxes_end.append(box_end)
        self.num_boundary_boxes = len(self.boundary_boxes_start)

        config_particle_num = fluid_particle_num + rigid_particle_num

        if max_num_particles is not None:
            # The total number of particles will be those from the config plus those added externally
            self.particle_max_num = config_particle_num + max_num_particles
        else:
            self.particle_max_num = config_particle_num

        if self.particle_max_num == 0:
            print("WARN: No particles defined. Allocating a dummy buffer of size 1 to prevent Taichi error.")
            self.particle_max_num = 1

        #### TODO: Handle the Particle Emitter ####
        # self.particle_max_num += emitted particles
        print(f"Total particles to allocate for: {self.particle_max_num}")
        ...
        # Allocate memory for custom boundary boxes if they exist
        if self.num_boundary_boxes > 0:
            self.boundary_boxes_start = ti.Vector.field(self.dim, dtype=float, shape=self.num_boundary_boxes)
            self.boundary_boxes_end = ti.Vector.field(self.dim, dtype=float, shape=self.num_boundary_boxes)
            for i in range(self.num_boundary_boxes):
                self.boundary_boxes_start[i] = self.boundary_boxes_start[i]
                self.boundary_boxes_end[i] = self.boundary_boxes_end[i]
        ...
        # Neighbor search properties (specifically for IISPH)
        self.fluid_neighbors = ti.field(int, shape=(self.particle_max_num, 100))
        self.solid_neighbors = ti.field(int, shape=(self.particle_max_num, 100))
        ...
        if self.simulation_method == 2 or self.simulation_method == 4:
    ...
    def build_solver(self):
        solver_type = self.simulation_method
        if solver_type == 0:
    ...
    def add_particles(self,
        ...
        # Update the count of fluid or solid particles
        if new_particles_num > 0:
            # We assume all particles in one batch are of the same material
            if new_particles_material[0] == self.material_fluid:
                self.fluid_particle_num += new_particles_num
            else:
                self.solid_particle_num += new_particles_num
    ...
    def counting_sort(self):
        ...
        for I in ti.grouped(self.grid_ids):
            ...
            if ti.static(self.simulation_method == 2 or self.simulation_method == 4):
