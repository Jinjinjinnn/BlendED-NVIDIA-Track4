...
class SPHBase:
    ...
    @ti.func
    def collide_with_box(self, p, pos, vel, box_start, box_end):
        """
        Custom box collision handling.
        This function will be called for each bounding_box defined in the config.
        """
        for i in ti.static(range(self.ps.dim)):
            if pos[i] < box_start[i]:
                pos[i] = box_start[i]
                if vel[i] < 0:
                    vel[i] *= -self.boundary_handling_method
            if pos[i] > box_end[i]:
                pos[i] = box_end[i]
                if vel[i] > 0:
                    vel[i] *= -self.boundary_handling_method
    ...
    def substep(self):
        for p in ti.grouped(self.ps.x):
            if self.ps.material[p] == self.ps.material_fluid:
                # Collision with domain boundary
                pos = self.ps.x[p]
                vel = self.ps.v[p]
                for i in ti.static(range(self.ps.dim)):
                    if pos[i] < self.ps.domain_start[i]:
                        pos[i] = self.ps.domain_start[i]
                        if vel[i] < 0:
                            vel[i] *= -self.boundary_handling_method
                    if pos[i] > self.ps.domain_end[i]:
                        pos[i] = self.ps.domain_end[i]
                        if vel[i] > 0:
                            vel[i] *= -self.boundary_handling_method
                
                # Collision with custom bounding boxes
                for i in ti.static(range(self.ps.num_boundary_boxes)):
                    box_start = self.ps.boundary_boxes_start[i]
                    box_end = self.ps.boundary_boxes_end[i]
                    pos, vel = self.collide_with_box(p, pos, vel, box_start, box_end)
